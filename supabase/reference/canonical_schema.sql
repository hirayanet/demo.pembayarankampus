-- Canonical schema (reference only, not auto-applied)
-- Safe to run in a clean environment; uses IF EXISTS/IF NOT EXISTS where possible.
-- Derived from migrations in supabase/migrations/.
...
-- updated_at trigger for bill_categories
...
create table if not exists public.bills (
  ...
  category text null, -- legacy, kept nullable and will be nullified by trigger
...
-- FK with ON DELETE SET NULL for category_id
...
-- Helpers to check roles (SECURITY DEFINER)
...
-- Students
drop policy if exists "AdminStaff read students" on public.students;
drop policy if exists "AdminStaff manage students" on public.students;
...
-- Bills
drop policy if exists "AdminStaff read bills" on public.bills;
drop policy if exists "AdminStaff manage bills" on public.bills;
drop policy if exists "Students read own bills" on public.bills;
...
-- Payments
drop policy if exists "AdminStaff read payments" on public.payments;
drop policy if exists "AdminStaff manage payments" on public.payments;
...
-- Bill categories
drop policy if exists "Categories read for all authenticated" on public.bill_categories;
drop policy if exists "Admins manage categories" on public.bill_categories;
